...
<body><?php include("../Navbar.php"); ?>
    <div class="container">
        <div class="header">
            <h1>Kategori</h1>
            <a class="btn" href="tambahKategori.php">+ Tambah</a>
        </div>
                ...
                <tbody>
                    <?php if (!$list): ?><tr>
                        ...
                        <?php else: foreach ($list as $row): ?>
                            <tr>
                                <td><?= (int)$row['idkategori'] ?></td>
                                <td><span class="badge"><?= htmlspecialchars($row['nama_kategori']) ?></span></td>
                                <td class="actions">
                                    <a class="btn secondary" href="updateKategori.php?id=<?= $row['idkategori'] ?>">Edit</a>
                                    <a class="btn danger" href="readKategori.php?id=<?= $row['idkategori'] ?>" onclick="return confirm('Hapus data ini?')">Hapus</a>
                                </td>
                            </tr>
                    <?php endforeach;
                    endif; ?>
